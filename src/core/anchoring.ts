...
export function relocateAnchor(
  ...
  const nearbyResult = searchRange(
    lines,
    Math.max(1, anchor.line - searchRadius),
    Math.min(lines.length, anchor.line + searchRadius),
    anchor
  );
...
function checkPositionMatch(
  ...
  lineNumber: number,
  anchor: Anchor
): { score: number; contentChanged: boolean } {
  ...
  const afterLines = getContextLines(
    lines,
    lineNumber + (anchor.endLine ? anchor.endLine - anchor.line : 0),
    CONTEXT_LINES,
    'after'
  );
  ...
  const annotatedLines = lines.slice(
    lineNumber - 1,
    lineNumber + (anchor.endLine ? anchor.endLine - anchor.line : 0)
  );
